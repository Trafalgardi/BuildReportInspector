        ...
        private void DisplayAssetsView()
        {
            ...
            switch (m_sourceDispMode)
            {
                case SourceAssetsDisplayMode.Size:
                    ...
                    int totalItems = m_contentAnalysis.m_assets.Count;
                    int totalPages = Mathf.Max(1, Mathf.CeilToInt((float)totalItems / m_itemsPerPage));
                    m_currentPage = Mathf.Clamp(m_currentPage, 0, totalPages - 1);

                    var pageAssets = m_contentAnalysis.m_assets
                        .Skip(m_currentPage * m_itemsPerPage)
                        .Take(m_itemsPerPage);
                    ShowAssets(pageAssets, ref vPos);

                    if (totalItems > m_itemsPerPage)
                    {
                        EditorGUILayout.Space();
                        EditorGUILayout.BeginHorizontal();

                        GUI.enabled = m_currentPage > 0;
                        if (GUILayout.Button("Prev", GUILayout.Width(60)))
                            m_currentPage--;

                        GUI.enabled = m_currentPage < totalPages - 1;
                        if (GUILayout.Button("Next", GUILayout.Width(60)))
                            m_currentPage++;

                        GUI.enabled = true;
                        GUILayout.FlexibleSpace();
                        GUILayout.Label($"Page {m_currentPage + 1} of {totalPages}");
                        EditorGUILayout.EndHorizontal();
                    }
                    break;
